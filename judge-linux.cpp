...
int getCurrentUserdMemory(pid_t);
...
int main(int argc, char *argv[], char **env) {
    ...
    const char* const shortOptions = "ho:v";
...
int runProcess(pid_t pid, sigset_t sigset,const string& commandLine, int timeLimit, int memoryLimit, int& usedTime, int& usedMemory) {
    ...
    if (pid > 0) {
        ...
        while (waitpid(pid, &exitCode, WNOHANG) != -1) {
            ...
            // Check memory limit
            int currentUsedMemory = getCurrentUserdMemory(pid);
...
bool isCurrentUsedMemoryIgnored(int currentUsedMemory, int memoryLimit) {
    int jvmUsedMemory = getCurrentUserdMemory(getpid());
 ...
 * @return 当前无力内存使用量(KB)
 */
int getCurrentUserdMemory(pid_t pid) {
